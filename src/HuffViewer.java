...
public class HuffViewer {
	...
	private VBox getInfo(double[] times, File[] originalFiles, File[] newFiles) {
		...
		VBox info = new VBox(INTERNAL_PADDING);
		...
		for (int i = 0; i < times.length; i++) {
			...
			info.getChildren().add(new Text(originalFiles[i].getName() + " -> " + newFiles[i].getName()));
			info.getChildren().add(new Text("\tTime: " + times[i] / 1000.0 + "s"));
			info.getChildren().add(new Text("\tOriginal length: " + originalFiles[i].length() + " bytes"));
			info.getChildren().add(new Text("\tNew length: " + newFiles[i].length() + " bytes"));
			double saved = 1.0 - ((double) newFiles[i].length() ) / ((double) originalFiles[i].length());
			info.getChildren().add(new Text("\t" + String.format("Percent space saved %.2f", saved * 100.0) + "%"));
			info.getChildren().add(new Text());
		...
		try {
			...
			Thread thread = new Thread(() -> {
				...
				updater.setGenerator(() -> {
					return in.bitsRead() / (8.0 * original.length());
				});
	...
	private void compare(File fileA, File fileB, StatusBar status, HuffPanel panel) {
		...
		Thread thread = new Thread(() -> {
			updater.setGenerator(() -> {
				return inA.bitsRead() / (8.0 * fileA.length());
			});
		...
		Thread thread = new Thread(() -> {
			...
			for (int i = 0; i < toCompress.size(); i++) {
				...
				try {
					...
					updater.setGenerator(() -> {
						return (updater.progress() + in.bitsRead()) / (8.0 * sum);
					});
					Processor processor = getProcessor();
					double start = System.currentTimeMillis();
					processor.compress(in, out);
